...
`
	...
	assert.Equal(t, Token{Type: DecOctave, Submatch: []string{}, Content: `<`, Row: 2, Col: 16}, next())
	...
	assert.Equal(t, Token{Type: DecOctave, Submatch: []string{}, Content: `<`, Row: 5, Col: 20}, next())
	...
	assert.Equal(t, Token{Type: CloseKey, Submatch: []string{}, Content: `}`, Row: 6, Col: 11}, next())
	assert.Equal(t, Token{Type: Number, Submatch: []string{"3"}, Content: `3`, Row: 6, Col: 12}, next())
